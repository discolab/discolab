...
function readJsonFile(filePath) {
  return readFile(filePath)
    ...
    .catch((e) => console.log('error reading JSON file:', e.toString()));
}
...
function findFiles(globPattern, dir) {
  return new Promise((resolve, reject) => {
    console.log(`looking for ${globPattern} files in ${dir}`);
    const globOptions = {
      cwd: dir,
      nocase: true
    };
...
function findJsons(globPattern, dir) {
  return findFiles(globPattern, dir).then((jsonFiles) => {
    ...
    console.log(`found ${sortedJsonFiles.length} ${globPattern} files`);
